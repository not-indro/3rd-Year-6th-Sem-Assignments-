...
#include<sys/ipc.h>
#include<unistd.h>
#include<sys/shm.h> // include shared memory header
#include<sys/sem.h>
#include<time.h>

#define PROJ_ID 1

struct att{ // This is the structure which is used to store the roll number and time
	int roll;
	time_t tm;
};
...
int main(int argc,char* argv[]){ // This is the main function
	...
	key = ftok("./Key_h/abc.txt",1);
//	key = ftok("/Key-Helper.txt",PROJ_ID);
	if(key == -1){ // This is the condition to check if the key is created or not
		perror("ftok: ");  // This is the error message
		return 1;
	}
	...
	struct att *shm_ptr; // This is the pointer which will point to the shared memory
